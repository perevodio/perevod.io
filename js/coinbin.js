		...
		if(account.match(/\+[0-9]{11}/g)){
			if($("#openPass").val().length>=10){
				if($("#openPass").val()==$("#openPassConfirm").val()){
					...
					$("#openLoginStatus").html("Your secret words do not match!").removeClass("hidden").fadeOut().fadeIn();
				}
			} else {
				$("#openLoginStatus").html("Your secret word must be at least 10 chars long").removeClass("hidden").fadeOut().fadeIn();
			}
		} else {
			$("#openLoginStatus").html("Your phone number doesn't appear to be valid").removeClass("hidden").fadeOut().fadeIn();
		}
			...
			if(dvalue>=total){
				...
				tx2.broadcast(function(data){
					if($(data).find("result").text()=="1"){
						$("#walletSendConfirmStatus").removeClass('hidden').addClass('alert-success').html("Sent!<br>txid: "+$(data).find("txid").text());
					} else {
		...
		$.ajax ({
			...
			error: function(data) {
				...
				if($(data).find("result").text()==1){
					...
					$("#rawTransactionStatus").html('Sent!<br>txid: '+$(data).find("txid").text());
				} else {
		...
		$.ajax ({
			...
			error: function(data) {
				...
				if((obj.status && obj.data) && obj.status=='success'){
					$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html('Sent!<br>txid: '+obj.data);
				} else {
		...
		$.ajax ({
			...
			error: function(data) {
				...
				if(obj.status && obj.txid){
					$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html('Sent!<br>txid: '+obj.txid);
				} else {
					$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(' Unexpected error, please try again').prepend('<span class="glyphicon glyphicon-exclamation-sign"></span>');
				}				
			},
		...
		$.ajax ({
			...
			error: function(data) {
				...
				if((obj.tx) && obj.tx.hash){
					$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(' Sent!<br>txid: '+obj.tx.hash);
				} else {
					$("#rawTransactionStatus").addClass('alert-danger').removeClass('alert-success').removeClass("hidden").html(' Unexpected error, please try again').prepend('<span class="glyphicon glyphicon-exclamation-sign"></span>');
				}				
			},
